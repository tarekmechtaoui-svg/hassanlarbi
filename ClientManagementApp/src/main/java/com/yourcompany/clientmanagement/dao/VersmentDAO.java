...
public class VersmentDAO {
    ...
    // Get all versments
    public List<Versment> getAllVersments(int sessionId) {
        List<Versment> versments = new ArrayList<>();
        String sql = "SELECT * FROM versement WHERE session_id = ? ORDER BY date_paiement DESC";
    ...
    // Get all versments from all sessions (admin function)
    public List<Versment> getAllVersmentsAllSessions() {
        List<Versment> versments = new ArrayList<>();
        String sql = "SELECT * FROM versement ORDER BY session_id DESC, date_paiement DESC";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            System.err.println("Error fetching versments from all sessions: " + e.getMessage());
        }
    ...
    public List<Versment> getVersmentsByClientId(int clientId) {
        List<Versment> versments = new ArrayList<>();
        String sql = "SELECT v.* FROM versement v " +
                    "INNER JOIN clients c ON v.client_id = c.id " +
                    "WHERE v.client_id = ? AND v.session_id = c.session_id " +
    ...
    public int insertVersment(Versment versment) {
        String sql = "INSERT INTO versement (client_id, montant, type, date_paiement, annee_concernee, session_id, created_at) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            stmt.setInt(6, versment.getSessionId());
            stmt.setTimestamp(7, Timestamp.valueOf(versment.getCreatedAt()));
    ...
    public boolean updateVersment(Versment versment) {
        String sql = "UPDATE versement SET client_id=?, montant=?, type=?, date_paiement=?, annee_concernee=?, session_id=? "
                + "WHERE id=?";
    ...
    public BigDecimal getTotalVersmentsByClientId(int clientId) {
        String sql = "SELECT SUM(v.montant) as total FROM versement v " +
                    "INNER JOIN clients c ON v.client_id = c.id " +
                    "WHERE v.client_id = ? AND v.session_id = c.session_id";
    ...
    private Versment mapResultSetToVersment(ResultSet rs) throws SQLException {
        ...
        versment.setSessionId(rs.getInt("session_id"));
